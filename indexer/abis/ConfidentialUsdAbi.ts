// ConfidentialUSD (ERC-7984 wrapper) ABI, exported `as const` so viem/Ponder can infer
// event arg types. Extracted from the Foundry artifact
// `contracts/out/ConfidentialUSD.sol/ConfidentialUSD.json` (the `.abi` field). Kept as a
// standalone file so the indexer doesn't depend on Foundry's git-ignored `out/`.
// Regenerate after a contract ABI change by re-extracting that artifact's `.abi`.
